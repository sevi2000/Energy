...
public class Common {
  ...
  public static void drawRotatedImage(
      Graphics g, int x, int y, int width, int height, int angle, Image img) {
  ...
  public static IntPair getClosestPolygon(BoardView bv, int x, int y) {
    ...
    for (Map.Entry<IntPair, IntPair> entry : bv.getCoordinateMap().entrySet()) {
      double distance =
          Math.sqrt(Math.pow(entry.getKey().a - x, 2) + Math.pow(entry.getKey().b - y, 2));
  ...
  public static void showConfirmation(
      String title, String msg, JFrame parentWindow, Level lvl, boolean stayHere) {
