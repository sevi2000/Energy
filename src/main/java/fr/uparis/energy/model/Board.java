...
public class Board implements BoardObservable, ReadOnlyBoard {
   ...
   */
  public void setRow(int lineNumber, List<Tile> row, boolean setNeighbors)
  ...
  public void propagateEnergy() {
    ...
    for (Tile t : this.getTilesWithComponent(Component.WIFI))
      if (t.isPowered()) oneWifiIsPowered = true;
      ...
      for (int j = 0; j < this.getWidth(); j++) {
        ...
        for (Connector c : t.getConnectors()) {
          try {
            c.setNeighbor(
                tileGrid
                    .get(i + c.getDirection().getHeightOffset(j))
